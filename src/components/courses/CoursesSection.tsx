...
function CourseCard({ course, currencySymbol }: { course: Course; currencySymbol: string }) {
  return (
    <article className="mx-auto flex w-full max-w-[405px] flex-col overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-[0_4px_4px_0_rgba(30,41,59,0.03),0_4px_4px_0_rgba(30,41,59,0.06)] transition hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-[16px] font-bold leading-tight text-[#1E293B]">{course.title}</h3>
          <span className="shrink-0 rounded-[26px] border-[0.5px] border-[#A7B6C6] bg-[#FFF3E4] px-3 py-1 text-[11px] font-semibold text-[#1E293B]">
            {course.categoryLabel}
          </span>
        </div>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px] font-medium text-[#475569]">
        ...
        <div className="mt-1 flex items-center justify-between gap-3">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] font-semibold text-[#475569]">
              {course.savePercent ? (
                <span className="inline-flex items-center gap-1">
          ...
          <Link
            href={course.href}
            className="inline-flex w-[156px] shrink-0 items-center justify-center gap-2 rounded-lg border border-[#F49114] bg-white px-4 py-[11px] text-[14px] font-semibold text-[#F49114] shadow-[0_4px_4px_0_rgba(244,145,20,0.11),0_4px_4px_0_rgba(244,145,20,0.11),0_4px_4px_0_rgba(244,145,20,0.08),0_4px_4px_0_rgba(244,145,20,0.03)] transition hover:bg-[#FFF7E8]"
          >
            View Course
    ...
    >
      <div className="site-container relative z-10 px-4 sm:px-6 lg:px-0">
        ...
        <div
          ...
          className="mt-8 flex flex-wrap items-center justify-between gap-2 rounded-lg bg-[#FCFCFC] p-2 shadow-[0_4px_4px_0_rgba(30,41,59,0.08),4px_-4px_4px_0_rgba(30,41,59,0.03)] md:mt-10"
        >
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`rounded-lg px-4 py-2.5 text-center text-[16px] font-medium leading-[140%] transition ${
                  isActive
                    ? 'bg-[#F49114] text-white shadow-sm'
        ...
        {filteredCourses.length === 0 ? (
          <p className="mt-10 text-center text-[14px] text-[#788593]">No courses available in this category yet.</p>
        ) : null}
        ...
        {hasMore ? (
          <div className="mt-10 flex justify-center">
            <button
              ...
              className="inline-flex items-center gap-2 text-[14px] font-semibold text-[#1E293B] underline-offset-4 transition hover:underline"
            >
              {viewMoreLabel}
            </button>
          </div>
