...
function DownloadIcon({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <defs>
        <clipPath id="course-content-download-clip">
...
export default function CourseContentSection({
  ...
  courseContent: CourseBodyContent['courseContent'];
}) {
  const [openModuleId, setOpenModuleId] = useState(courseContent.modules[0]?.id ?? '');
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-[34px] font-bold leading-[140%] text-heading">{courseContent.title}</h2>
        <CourseBrochureCta
          href="#brochure"
        ...
        >
          {courseContent.downloadSyllabusLabel}
      ...
      <div className={`mt-5 ${COURSE_INNER_CARD}`}>
          {courseContent.modules.map((module, index) => {
            ...
            const isLastModule = index === courseContent.modules.length - 1;
              ...
              >
                ...
                {isOpen ? (
                  <div className="flex gap-3 px-5 pb-5 md:px-6">
                    ...
                    <div className="min-w-0 flex-1 rounded-lg bg-[#F8F9FB] p-5 md:p-6">
                      <h4 className="text-[13px] font-semibold text-heading">Learning Objectives</h4>
                      <div className="mt-2 space-y-2">
                        {module.learningObjectives.map((paragraph) => (
                      ...
                      <ul className="mt-2 space-y-1.5" role="list">
                        {module.topicsCovered.map((topic) => (
          ...
          <div className={`${COURSE_TOP_DIVIDER} px-5 py-4 text-center md:px-6`}>
            <Link
              href={courseContent.viewMoreHref}
              className="group inline-flex items-center gap-2 text-[14px] font-medium leading-[18px] text-brand transition hover:underline"
            >
              {courseContent.viewMoreLabel}
