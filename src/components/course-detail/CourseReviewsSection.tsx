...
import type { CourseLearnerReview, CourseReviewsContent } from '@/lib/courseBody';
...
function ReviewCard({ review }: { review: CourseLearnerReview }) {
  return (
    <article className={REVIEW_CARD}>
      <div className="absolute top-0 left-5 z-10 -translate-y-1/2">
        <QuoteIcon />
      </div>
      <h3 className="mt-3 text-[14px] font-semibold leading-normal text-heading">{review.title}</h3>
      <p className="mt-2 line-clamp-3 flex-1 text-[13px] font-normal leading-normal text-muted">
        {review.quote}
      </p>
      <Link
        href={review.readOnHref}
      ...
      >
        ...
        <span className="relative h-4 w-16 shrink-0">
          <Image
            src={review.readOnLogoSrc}
            alt={review.readOnLogoAlt}
            fill
      ...
      <div className="mt-4 border-t border-[#EBEBEB] pt-4">
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate text-[14px] font-medium leading-[20px] text-heading">
              {review.name}
            </p>
            <p className="mt-0.5 truncate text-[12px] font-normal leading-normal text-muted">
              {review.role}
            </p>
...
function VideoCard({ video }: { video: CourseReviewsContent['video'] }) {
  return (
    <div className="relative h-[282px] w-full max-w-[295px] overflow-hidden rounded-[20px] shadow-[0_4px_4px_0_rgba(30,41,59,0.08),0_4px_4px_0_rgba(30,41,59,0.03)]">
      ...
      />
      <div
        className="absolute inset-0 bg-[rgba(30,41,59,0.40)]"
        aria-hidden
      />
      <Link
        href={video.videoUrl ?? '#'}
...
export default function CourseReviewsSection({ reviews }: { reviews: CourseReviewsContent }) {
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.reviews.filter((review) => review.platform === activeTabId);
  }, [activeTabId, reviews.reviews]);

  const carouselItems = useMemo(
    () => [{ type: 'video' as const }, ...filteredReviews.map((review) => ({ type: 'review' as const, review }))],
    [filteredReviews],
  );
  ...
  return (
    <div id="reviews" className={`scroll-mt-[116px] py-5 md:py-6 ${SECTION_CARD}`}>
        ...
        {pages.map((pageItems, pageIndex) => (
          <div key={pageIndex} className="grid grid-cols-1 gap-4 overflow-visible md:grid-cols-3 md:gap-5">
            {pageItems.map((item, itemIndex) =>
              item.type === 'video' ? (
                <VideoCard key={`video-${pageIndex}-${itemIndex}`} video={reviews.video} />
              ) : (
      ...
      <div className="mt-5 flex flex-col overflow-hidden rounded-[20px] border border-[#EBEBEB] bg-white shadow-[0_4px_4px_0_rgba(30,41,59,0.08),0_4px_4px_0_rgba(30,41,59,0.03)] sm:flex-row">
        {reviews.platformRatings.map((platform) => (
          <div
            key={platform.id}
            className="flex flex-1 flex-col items-center justify-center gap-2 px-4 py-4 sm:px-5"
          >
            ...
            <div className="flex min-w-0 items-center justify-center gap-2">
              ...
              <span className="text-[11px] font-normal leading-normal text-muted">
