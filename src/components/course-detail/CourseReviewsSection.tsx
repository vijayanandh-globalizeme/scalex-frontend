...
function ReviewCard({ review, typeMap }: { review: ApiReview; typeMap: Map<string, typeof REVIEWS_TYPE[number]> }) {
...
// Map review types to platform rating keys in settings
const PLATFORM_KEYS: { typeId: string; settingKey: keyof LayoutSettings }[] = [
  { typeId: 'GOOGLE_REVIEW',       settingKey: 'GOOGLE_REVIEW' },
  { typeId: 'FACEBOOK_REVIEW',     settingKey: 'FACEBOOK_REVIEW' },
  { typeId: 'TRUST_PILOT_REVIEW',  settingKey: 'TRUST_PILOT_REVIEW' },
  { typeId: 'SWTICH_UP_REVIEW',    settingKey: 'SWTICH_UP_REVIEW' },
];
...
export default function CourseReviewsSection({
  ...
  // Build tabs: always "All", then only types that actually appear in reviews
  ...
  // Build platform ratings from settings, only show those with data
  const platformRatings = useMemo(() =>
    PLATFORM_KEYS
      .map(({ typeId, settingKey }) => {
        const entry = settings[settingKey] as { rating: string; count: number } | undefined;
        if (!entry) return null;
        const meta = typeMap.get(typeId);
        if (!meta) return null;
        return { id: typeId, label: meta.label, logoSrc: meta.logoSrc, logoAlt: meta.logoAlt, rating: entry.rating, reviewsLabel: `${entry.count} Reviews` };
      })
      .filter(Boolean) as { id: string; label: string; logoSrc?: string; logoAlt?: string; rating: string; reviewsLabel: string }[],
    [settings, typeMap],
  );
          ...
          {platformRatings.map((platform) => (
            <div key={platform.id} className="flex flex-1 flex-col items-center justify-center gap-2 px-4 py-4 sm:px-5">
              {platform.logoSrc ? (
                ...
                <span className="text-[13px] font-semibold text-heading">{platform.label}</span>
              )}
