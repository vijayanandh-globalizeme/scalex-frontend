...
import type { ApiOtherDetail } from '@/services/courseApi';
import {
  CAREER_SECTION_TITLE,
  CAREER_SALARY_SUBTITLE,
  CAREER_SALARY_BADGE,
  CAREER_COMPANIES_SUBTITLE,
  CAREER_COMPANIES_BADGE,
  CAREER_DEMAND_SUBTITLE,
  CAREER_DEMAND_BADGE,
  HIRING_COMPANIES,
  COURSE_OVERVIEW_DEMAND_LABEL,
...
function ColumnHeader({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle: string }) {
  return (
...
function RupeeIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="14" viewBox="0 0 10 14" fill="none" aria-hidden>
      ...
      <defs><clipPath id="course-career-rupee-clip"><rect width="10" height="14" fill="white" /></clipPath></defs>
    </svg>
  );
}
    ...
    <div className="flex flex-col items-center">
      <div className="relative h-[95px] w-[95px]">
        <svg width="95" height="95" viewBox="0 0 108 108" className="-rotate-90" aria-hidden>
          ...
          <circle cx="54" cy="54" r={radius} fill="none" stroke="#FD022D" strokeWidth="8"
...
export default function CourseCareerSection({
  ...
  const [activeTabId, setActiveTabId] = useState(careerTabs[0]?.id ?? '');
  const activeTab = careerTabs.find((t) => t.id === activeTabId) ?? careerTabs[0];
        ...
        >
          <div className="grid md:grid-cols-3 md:items-stretch">
            ...
            <div className="flex min-w-0 flex-col p-5 md:p-6">
              <ColumnHeader icon={<RupeeIcon />} title="Average Salary" subtitle={CAREER_SALARY_SUBTITLE} />
              <div className="relative mt-5 flex-1">
                {activeTab.file ? (
                  <Image
                    src={activeTab.file.url}
                    alt={`${activeTab.title} salary chart`}
                    width={400}
                    height={148}
              ...
              <CareerBadge>{CAREER_SALARY_BADGE}</CareerBadge>
            </div>

            {/* Companies */}
            <div className={`flex min-w-0 flex-col p-5 md:p-6 ${CAREER_COLUMN_DIVIDER}`}>
              <ColumnHeader icon={<BuildingIcon />} title="Top Hiring Companies" subtitle={CAREER_COMPANIES_SUBTITLE} />
              ...
              <CareerBadge>{CAREER_COMPANIES_BADGE}</CareerBadge>
            </div>

            {/* Demand */}
            <div className={`flex min-w-0 flex-col p-5 md:p-6 ${CAREER_COLUMN_DIVIDER}`}>
              <ColumnHeader icon={<PeopleIcon />} title="Demand" subtitle={CAREER_DEMAND_SUBTITLE} />
              <div className="flex flex-1 flex-col items-center justify-center">
                <DemandRing percent={tab.percentage} label={`${tab.title} ${COURSE_OVERVIEW_DEMAND_LABEL}`} />
              </div>
              <CareerBadge>{CAREER_DEMAND_BADGE}</CareerBadge>
            </div>
          </div>
        </div>
      ))}
