...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const loc = locations.find((l) => l.countryUri === countryUri && l.cityUri === cityUri);
  const cityName = loc?.cityName ?? cityUri;

  const course = await getCourseOverview(courseUri, categoryUri, { countryUri, cityUri });
  if (!course) return { title: 'Course Not Found' };

  const title = `${course.seo?.metaTitle ?? course.title} in ${cityName}`;
...
export default async function CityCourseDetailPage({ params }: PageProps) {
  const { categoryUri, courseUri, countryUri, cityUri } = await params;

  const locations = await getCourseLocations(courseUri);
  const loc = locations.find((l) => l.countryUri === countryUri && l.cityUri === cityUri);
  if (!loc?.cityId) notFound();

  const [course, layoutData] = await Promise.all([
    getCourseOverview(courseUri, categoryUri, { countryUri, cityUri }),
    fetchLayout(),
  ]);
  ...
  const extraBreadcrumbs = [
    ...
    { label: loc.cityName!,   href: `/${categoryUri}/${courseUri}/${countryUri}/${cityUri}` },
  ];

  const scheduleBasePath = `/${categoryUri}/${courseUri}/${countryUri}/${cityUri}`;
  ...
  if (isTechnical) {
    ...
    return (
      <CourseDetailPageShell form={DEFAULT_FORM}>
        ...
        <CourseDetailBodySection courseUri={courseUri} categoryUri={categoryUri} isTechnical phone={settings.CONTACT_WHATSAPP_NO} courseDetails={course} settings={settings} countryUri={countryUri} cityUri={cityUri} />
      </CourseDetailPageShell>
    );
  }
  ...
  return (
    <CourseDetailPageShell form={DEFAULT_FORM}>
      ...
      <CourseEnterpriseSection />
      <CourseDetailBodySection courseUri={courseUri} categoryUri={categoryUri} phone={settings.CONTACT_WHATSAPP_NO} courseDetails={course} settings={settings} countryUri={countryUri} cityUri={cityUri} />
    </CourseDetailPageShell>
  );
}
