  ...
  if (isTechnical) {
    ...
    return (
      <CourseDetailPageShell form={DEFAULT_FORM}>
        ...
        <CourseDetailBodySection courseUri={courseUri} categoryUri={categoryUri} isTechnical phone={settings.CONTACT_WHATSAPP_NO} syllabusUrl={course.details?.syllabus?.url ?? null} courseContentTitle={course.details?.courseContentTitle} faqTitle={course.details?.faqTitle} aboutTitle={course.details?.aboutTitle} aboutContent={course.details?.aboutContent} trainerTitle={course.details?.trainerTitle} reviewsTitle={course.details?.reviewsTitle} settings={settings} />
      </CourseDetailPageShell>
    );
  }
